...
const keyToLane = keyLayout === "alt"
  ...
  : keyLayout === "num"
  ? { "4": "lane0", "5": "lane1", "7": "lane2", "8": "lane3", "9": "lane4" }
...
if (notes.filter(n => n.active).length > 0) return;
if (notes.length < chart.length) return; 
...
if (storyUnlockNotice) {
  const unlockedByThisPlay =
    !wasPlayed &&
    (currentSong === "song5" || currentSong === "boss" || currentSong === "song2"|| currentSong === "song8"|| currentSong === "song10"|| currentSong === "song9");
...
document.addEventListener("keydown", async (e) => {
  ...
  keys[e.key] = true;
    ...
    if (storyChallengeIntroActive && e.key === " ") {
  e.preventDefault();
...
function setupPartnerSkill() {
  ...
  if (activePartnerSkill.type === "timedHeal") {
